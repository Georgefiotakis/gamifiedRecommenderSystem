$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: "./analytics/ageUserAnalytics",
        success: function (data, status) {
            buildUserAgeBarChart(data)
        },
...
// Create chart instance
    ...
    title.text = "How many users are from the same age";
    title.fontSize = 21;
    title.marginBottom = 30;

    for (var j = 0; j < data.length; j++) {
        tempData["data"][j] = {};
        tempData.data[j]["age"] = data[j].age;
...
function buildUserAgeBarChart(data) {

    // console.log("Data:" + data);
    am4core.useTheme(am4themes_animated);
    am4core.options.autoSetClassName = true;

    var chart = am4core.create("userBarChart", am4charts.XYChart);
    chart.scrollbarX = new am4core.Scrollbar();
    ...
    for (var j = 0; j < data.length; j++) {
        tempData2["data"][j] = {};
        tempData2.data[j]["age"] = data[j].age;
        tempData2.data[j]["users"] = data[j].users;
    }
